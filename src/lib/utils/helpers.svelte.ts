...
export const rgbToHex = (rgb: string) => {
	...
	return match[1].split(',').reduce((acc, val) => acc + Number(val).toString(16).slice(-2), '#')
}
...
export const oppositeColorNameToHex = (colorName: string) => {
	...
	return match.reduce((acc, val) => (255 - Number(val)).toString(16).slice(-2), '#')
}
...
export const hexCodeToOpposite = (hex_code: string) => {
	...
	const twoHexDigits = (ix: number, iy?: number) => {
		const d = 255 - hex.indexOf(hex_code[ix]) * 16 - hex.indexOf(hex_code[ix + 1])
...
const randomBackColor = (): string => {
	return colors_hex[Math.floor((Math.random() * 1000) % colorCount)]
}
...
export const rgbToOppositeHex = (rgb: string) => {
	...
	return match[1]
		?.split(',')
		.reduce((acc: string, val: string) => acc + (255 - Number(val)).toString(16).slice(-2), '#')
}
...
export const hexCodeToOpposite = (hex_code: string) => {
	...
	let val = '#'
	const twoHexDigits = (ix: number) => {
		const d =
			255 - hex.indexOf(hex_code[ix] as string) * 16 - hex.indexOf(hex_code[ix + 1] as string)
		const x = d.toString(16)
		return x === '0' ? '00' : x
	}
	val += ('0' + twoHexDigits(0)).slice(-2)
	val += ('0' + twoHexDigits(2)).slice(-2)
	val += ('0' + twoHexDigits(4)).slice(-2)
	return val
}
const colorCount = colors_hex.length
const randomBackColor = (): string => {
...
export const niceColor = (clr: string) => {
	let hsl = rgb2hsl(clr) as number[]
	hsl[0] = ((hsl[0] as number) + 0.5) % 1
	hsl[1] = ((hsl[1] as number) + 0.5) % 1
	hsl[2] = ((hsl[2] as number) + 0.5) % 1
	return 'hsl(' + hsl[0] * 360 + ',' + hsl[1] * 100 + '%,' + hsl[2] * 100 + '%)'
}
...
export const invertColor = (hex: string, bw?: boolean) => {
	...
	// convert 3-digit hex to 6-digits.
	if (hex.length === 3) {
		hex = (((hex[0] as string) + hex[0]) as string) + hex[1] + hex[1] + hex[2] + hex[2]
	}
	if (hex.length !== 6) {
		throw new Error('Invalid HEX color.')
	}
