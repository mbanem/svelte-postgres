<script lang="ts">
	...
	onMount(() => {
		message.innerHTML = msg
		inputEl.addEventListener('beforeinput', function (e: InputEvent) {
			if ('deleteContentForward|deleteContentBackward'.includes(e.inputType)) return
			const el = e.target as HTMLInputElement
			const nextVal =
				el.value.substring(0, Number(el.selectionStart)) +
				e.data +
				el.value.substring(Number(el.selectionEnd))
			if (!/^-?(\d{0,10}|\d{0,8}\.?\d{0,2}|\d{0,9}\.?\d{0,1}|)$/.test(nextVal)) {
				e.preventDefault()
...
<pre>
  ...
  allows only numeric types to be accepted (at Oct 23<sup>rt</sup> Firefox excluded)
</pre>
<br />
<input type="number" class="inputmode-numeric" inputmode="numeric" pattern="[0-9]*" />
