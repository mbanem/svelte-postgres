  ...
  const enhanceSubmit: SubmitFunction = async ({ action, formData }) => {
    ...
    return async ({ update }) => {
      ...
      if (action.search === '?/create') {
        result = page.status === 200 ? 'b-user created' : 'create failed';
      } else if (action.search === '?/update') {
        result = page.status === 200 ? 'b-user updated' : 'update failed';
      } else if (action.search === '?/delete') {
        result = page.status === 200 ? 'b-user deleted' : 'delete failed';
