<script lang="ts">
  ...
  */
  ...
  async function flip() {
    ...
    divEl.style.transitionDelay = divEl.classList.contains('transform-active')
      ? '0.5s'
      : '-0.1s';
    divEl.style.transitionDuration = divEl.classList.contains(
      'transform-active',
    )
      ? '1s'
      : '1s';
  ...
  const moveCard = (id: string) => {
    slider =
      id === '1' ? (slider1 as HTMLDivElement) : (slider2 as HTMLDivElement);
...
<div class="in-grid">
  <div class="caterpillar">
    click on square box to make it the last one in caterpillar
  </div>
  <div class="label-abs">
    <div data-top={layout} class="transform">
      click on an image to rearrange them
    </div>
  </div>
  <div class="slider1">
    {#each [...n5] as n}
      <!-- could use [...5] as well -->
      <div class="item" onclick={sliderOnClick} aria-hidden={true}>
        {n5 - n}
      </div>
    {/each}
  </div>
