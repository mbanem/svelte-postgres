<script lang="ts">
  ...
  const createChildObject = (familyObject: FamilyObject, childName: string) => {
    ...
    if (!children[fid]?.includes(childName))
      alert(`${childName} is not from this family`);
  ...
  const moveFamilyObject = (familyObject: FamilyObject, shift: number) => {
    ...
    const toIx =
      fromIx + shift > list.length || fromIx + shift < 0
        ? fromIx
        : fromIx + shift;
  ...
  const moveChildObject = (
    ...
    if (CFS < familyObject.childObjects.length && CFS >= 0) {
      ...
      CFS >= familyObject.childObjects.length &&
    ...
    const moved = list[familyFrom]?.childObjects.splice(
      childFrom,
      1,
    )[0] as ChildObject;
  ...
  const addChildToFamily = (familyObject: FamilyObject) => {
    ...
    const childName = (childrenSelBox.options[cid] as HTMLOptionElement)
      .innerText;
  ...
  const movingDisabled = (
    ...
    const cix = familyObject.childObjects.findIndex(
      (co) => co.text === childObject.text,
    );
    return (
